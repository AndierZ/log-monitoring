...
public class LogEntryHandler extends MessageHandler<LogEntryParser> {
    ...
    private final long totalTrafficCheckInterval;
    private final int totalTrafficThresholdPerSec;
    ...
    public LogEntryHandler(Context context) {
        ...
        this.totalTrafficCheckInterval = TimeUnit.SECONDS.toMillis((int) context.getConfig().getOrDefault("total_traffic_check_interval", 120));
        this.totalTrafficThresholdPerSec = (int) context.getConfig().getOrDefault("total_traffic_threshold_per_sec", 10);
    }
